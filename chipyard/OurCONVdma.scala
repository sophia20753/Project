...
class OurCONV(opcodes: OpcodeSet)(implicit p: Parameters) extends LazyRoCC(opcodes) {
	println("[Elab] OurCONV instantiated")
	override lazy val module = new OurCONVModuleImp(this)
}
	...
	*/
    ...
    when(state === sWait) {
        ...
        when(io.dataOut.fire) {
        	counter := counter + 1.U
        	printf(p"[DMA] Counter: ${counter}")
        	when(counter + 1.U === io.length) {
        		state := sDone
        	}
        }
    }
...
class OurCONVModuleImp(outer: OurCONV)(implicit p: Parameters) extends LazyRoCCModuleImp(outer)
	with HasCoreParameters {
		...
		dma.io.start := (state === sLoad)
		//dma.io.baseAddr := baseAddr 
		dma.io.length := totalKernelElements
		...
		switch(state) {
		    is(sIdle) {
		        when(cmd.valid && doLoadKernel) {
					...
					baseAddr := cmd.bits.rs2
					loadCounter := 0.U
					printf("[RoCC] sIdle -> sLoad: rd=%d, ksize=%d, base=0x%x\n", rd, kernelSize, baseAddr)
					state := sLoad
		        }
		    }

		    is(sLoad) {
				printf("[RoCC] sLoad: waiting for dma.done=%d\n", dma.io.done)
		        when(dma.io.done) {
		            state := sWaitMem 
		            printf("[RoCC] sLoad -> sWaitMem\n")
		        }
		    }
		    // TODO: DMA bulk memory transfer
		    is(sWaitMem) {
		        ...
		        when(dma.io.dataOut.valid && dma.io.dataOut.ready) {
					val row = loadCounter / kernelDim
					val col = loadCounter % kernelDim
					val data = dma.io.dataOut.bits
					printf("[RoCC] Receiving kernel[%d][%d] = %d\n", row, col, data)
					kernel(row)(col) := data
					loadCounter := loadCounter + 1.U 
					when(loadCounter + 1.U === totalKernelElements) {
						printf("[RoCC] Finished loading kernel\n")
						state := sDone
					}
				}
		    }
		    ...
		    is(sDone) {
				doneSignal := 1.U 
				printf("[RoCC] Kernel Load Complete. Matrix:\n")
				for (i <- 0 until 5) {
					for (j <- 0 until 5) {
						printf(p"${kernel(i)(j)} ")
					}
